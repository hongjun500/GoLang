...
> 变量 `i` 和 `j` 具有不同的静态类型，尽管它们具有相同的基础类型，但不能直接赋值，需要进行显示地类型转换 **j = MyInt(i)**
...
> **被断言的值必须是接口类型；断言目标 T 可以是具体类型或接口类型**

**断言的语法: `value, ok := x.(T)`,可以只返回一个值`value`，但是如果断言失败，那么会抛出一个 panic 异常**
...
- 需要注意 **类型别名** 和 **自定义类型** 的区别看似是有无 `=`，类型别名在语言层面就与目标类型是同一个类型（type identity 相同），它自身没有独立的类型身份，所以反射看到的必然是目标类型本身的名字；而自定义类型创造了一个全新的类型身份，这个新身份连同它的名字，会被编译器保留到运行时类型信息里，所以反射才能打印出这个新类型自己的 package.Name。两者的类型信息都存在于编译产物和运行时，区别不在于"存在与否"，而在于"是否是一个新的、独立的类型"。
