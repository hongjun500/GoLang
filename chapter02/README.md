...
- 目前循环只有 `for` (截至 v1.26.3)
- `for` 循环结构体包含初始化表达式，条件表达式，后置语句;
  用英文分号 `;` 隔开;其中只有条件表达式是必须包含的
- ~~后置语句也应该显示声明(比如使用 `break` 结束所在循环，`continue` 跳出循环，或者使用 `return` 退出循环)，否则容易陷入无限循环~~
- 如果条件表达式依赖的变量没有在后置语句里被修改，需要在循环体内手动修改，否则会死循环；也可以用 `break/return` 提前终止循环，用 `continue` 跳过本次循环剩余部分
...
- **`range` 关键字遍历字符串时的特殊处理**
> Go 字符串的本质：UTF-8 字节序列 Go 里的 string 本质上就是一段只读的字节序列（[]byte），并没有单独的"字符"类型来存储。当字符串里出现非 ASCII 字符（比如中文）时，Go 会用 UTF-8 编码，把一个字符编码成 1~4 个字节。
  1. 英文字母、数字、常见英文符号（ASCII 范围）：1 个字节
  2. 常见汉字（比如"你"、"好"）：通常是 3 个字节
  3. 一些特殊符号/emoji：可能是 2~4 个字节
- **`range` 关键字遍历循环返回两个值，第一个是索引，第二个是索引对应的值**
- `range` 关键字遍历循环可以使用 `_` 忽略索引或者值

- `range` **遍历循环时如果只取索引，可以直接省略第二个返回值**
- `range` 关键字遍历循环可以使用 `break` 结束循环，使用 `continue` 跳出一次循环，使用 `return` 退出循环
...
- 如果存在 `fallthrough` 语句(无法在**类型匹配**中使用)，那么其后必须包含一个 `case` 或者是 `default` ，否则编译不通过程序报错
...
- `case` 支持**多个表达式**，用逗号 `,` 隔开，表示只要有一个表达式匹配就会执行该 `case` 结构体代码，例如
  ```go
...
_对于上方的 un 函数执行顺序：un("b")的顺序先于 un("a"), 因此 `leaving: a` 先输出_
...
- 不能跳转到其他函数内的标签；不能跳过变量声明直接跳进它的作用域（会编译报错 goto jumps over declaration of ...）
- 不能跳入一个更深层的代码块内部。这些是初学者用 goto 时最容易遇到的编译错误。
